...
def pipe_blood(abf_blood):
    ...
    pipe = Pipeline(
        ...
        lowpass(cutoff=10e3, abf=abf_blood),
        trim(left=fs * 0.01),
        as_ires(),
...
def test_peptides():
    ...
    first = Pipeline(
        ...
        lowpass(cutoff=10e3, abf=abf),  # pass the segment through a lowpass filter at 10kHz
        as_ires(bl=bl),  # detect the baseline between the min and max amplitudes and calculate Ires
    )
    second = Pipeline(
        cusum(mu=1, sigma=sd / bl, omega=60, c=200, padding=50),  # event detection using cusum method
